...
class B2E_Migration_Service
{
    ...
    /** @var B2E_Media_Service */
    private $media_service;

    /** @var B2E_Content_Service */
    private $content_service;

    /** @var B2E_API_Client */
    private $api_client;

    /** @var B2E_Migrator_Registry */
     ...
     * @param B2E_Plugin_Detector              $plugin_detector
     * @param B2E_Content_Parser               $content_parser
     * @param B2E_CSS_Service                  $css_service
     * @param B2E_Media_Service                $media_service
     * @param B2E_Content_Service              $content_service
    ...
    public function __construct(
        ...
        B2E_CSS_Service $css_service,
        B2E_Media_Service $media_service,
        B2E_Content_Service $content_service,
        B2E_API_Client $api_client,
        B2E_Migrator_Registry $migrator_registry,
    ...
    public function start_migration($target_url, $api_key)
    {
        try {
            ...
            if (!$validation_result['valid']) {
                ...
                $this->error_handler->log_error('E103', [
            ...
            $this->update_progress(
                ...
                25,
                sprintf(
                    __('Found %d Bricks posts, %d Gutenberg posts, %d media files (%d total)', 'bricks-etch-migration'),
            ...
            return [
                ...
                'completed' => true,
                'message' => __('Migration completed successfully!', 'bricks-etch-migration'),
                'details' => [
                    'media' => $media_result,
                    'css' => $css_result,
                    'posts' => $posts_result,
            ...
            $this->error_handler->log_error('E201', [
    ...
    public function cancel_migration($migration_id = '')
    {
        ...
        $this->error_handler->log_warning('W900', [
            'action' => 'Migration cancelled by user',
        ]);

        return [
            'message' => __('Migration cancelled.', 'bricks-etch-migration'),
            'progress' => $this->get_progress_data(),
            'steps' => $this->get_steps_state(),
            'completed' => false,
        ];
    ...
    public function validate_target_site_requirements()
    {
        ...
        if (!is_array($result)) {
            return [
                'valid' => false,
                'errors' => [__('Unknown validation response.', 'bricks-etch-migration')],
            ];
        }
    ...
    public function start_import_process($source_domain, $token)
    {
        ...
        return new \WP_Error('not_supported', __('Start import process is not available on the current API client version.', 'bricks-etch-migration'));
    }
    ...
    private function initialize_steps()
    {
        ...
        foreach ($supported as $migrator) {
            $steps[$this->get_migrator_step_key($migrator)] = ['status' => 'pending'];
        }
    ...
    {
        $this->error_handler->log_error('I010', [
    ...
    private function execute_migrators($target_url, $api_key)
    {
        ...
        if (empty($supported_migrators)) {
            $this->error_handler->log_warning('W002', [
        ...
        foreach ($supported_migrators as $migrator) {
            ...
            $this->update_progress($step_key, $progress, sprintf(__('Migrating %s...', 'bricks-etch-migration'), $migrator->get_name()));
...
class_alias(B2E_Migration_Service::class, 'B2E_Migration_Service');
