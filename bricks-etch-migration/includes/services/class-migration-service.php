...
class B2E_Migration_Service {
	...
	public function start_migration( $target_url, $api_key ) {
		try {
			...
			$this->update_progress(
				...
				25,
				sprintf(
					__( 'Found %1$d Bricks posts, %2$d Gutenberg posts, %3$d media files (%4$d total)', 'bricks-etch-migration' ),
	...
	public function validate_target_site_requirements() {
		...
		if ( ! is_array( $result ) ) {
			return array(
				'valid'  => false,
				'errors' => array( __( 'Unknown validation response.', 'bricks-etch-migration' ) ),
			);
		}
