...
use Bricks2Etch\Controllers\B2E_Dashboard_Controller;
use Bricks2Etch\Controllers\B2E_Settings_Controller;
use Bricks2Etch\Controllers\B2E_Migration_Controller;
...
class B2E_Admin_Interface {
	...
	public function __construct(
		B2E_Dashboard_Controller $dashboard_controller,
		B2E_Settings_Controller $settings_controller,
		B2E_Migration_Controller $migration_controller,
		$register_menu = true
	) {
	...
	public function enqueue_admin_assets( $hook ) {
		...
		wp_enqueue_script(
			'b2e-admin-main',
			B2E_PLUGIN_URL . 'assets/js/admin/main.js',
			array(),
			B2E_VERSION,
			true
		);
	...
	private function verify_request() {
		if ( ! check_ajax_referer( 'b2e_nonce', 'nonce', false ) ) {
			wp_send_json_error( __( 'Invalid request.', 'bricks-etch-migration' ) );
			return false;
		}
...
\class_alias( __NAMESPACE__ . '\\B2E_Admin_Interface', 'B2E_Admin_Interface' );
