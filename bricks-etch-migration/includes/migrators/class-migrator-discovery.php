...
class B2E_Migrator_Discovery {
    ...
    public static function discover_migrators(B2E_Migrator_Registry $registry) {
        ...
        foreach ($builtin_keys as $type => $key) {
            ...
            if ($migrator instanceof Migrator_Interface) {
                try {
                    ...
                    if ($error_handler) {
                        $error_handler->log_warning('W002', array(
                            'migrator_type' => $type,
        ...
        if ($error_handler) {
            $error_handler->debug_log('Migrators discovered', array(
...
\class_alias(__NAMESPACE__ . '\\B2E_Migrator_Discovery', 'B2E_Migrator_Discovery');
