...
class EFS_API_Endpoints {
	...
	/**
	 * Apply rate limit helper for template endpoints.
	 *
	 * @param string $action Action key.
	 * @param int    $limit  Requests per minute.
	 * @return \WP_Error|bool
	 */
	private static function enforce_template_rate_limit( $action, $limit = 20 ) {
		return self::check_rate_limit( 'template_' . $action, $limit );
	}
	...
	public static function register_routes() {
		...
		register_rest_route(
			$namespace,
			'/export/migrator/(?P<type>[a-z0-9_\-]+)',
			array(
				'methods'             => 'GET',
