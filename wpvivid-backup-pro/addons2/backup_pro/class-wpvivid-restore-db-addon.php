...
class WPvivid_Restore_DB_Addon
{
    ...
    public function restore($sub_task,$backup_id)
    {
        ...
        if($sub_task['unzip_file']['unzip_finished']==0)
        {
            ...
            if(isset($files[0]['options']['is_crypt'])&&$files[0]['options']['is_crypt']=='1' || isset($files[0]['options']['is_crypt_ex'])&&$files[0]['options']['is_crypt_ex']=='1' || $is_crypt === true)
            {
                usort($files, function ($a, $b)
                {
                    if(isset($a['parent_file']) && isset($b['parent_file']))
                    {
                        preg_match('/part(\d+)(?=of\d*\.zip|\.zip)/', $a['parent_file'], $matches);
                        if (isset($matches[1])) {
                            $a_tmp = $matches[1];
                        } else {
                            $a_tmp = 0;
                        }
                        preg_match('/part(\d+)(?=of\d*\.zip|\.zip)/', $b['parent_file'], $matches);
                        if (isset($matches[1])) {
                            $b_tmp = $matches[1];
                        } else {
                            $b_tmp = 0;
                    ...
                    {
                        preg_match('/part(\d+)(?=of\d*\.zip|\.zip)/', $a['file_name'], $matches);
                        if (isset($matches[1])) {
                            $a_tmp = $matches[1];
                        } else {
                            $a_tmp = 0;
                        }
                        preg_match('/part(\d+)(?=of\d*\.zip|\.zip)/', $b['file_name'], $matches);
                        if (isset($matches[1])) {
                            $b_tmp = $matches[1];
                        } else {
                            $b_tmp = 0;
                ...
                foreach ($files as $file)
                {
                    ...
                    foreach ($sql_files as $tmp_sql_file)
                    {
                        ...
                        $ret=$this->restore_crypt_db($tmp_sql_file['file_name'],$local_path,$sub_task);
                        if($ret['result']!='success')
                        {
                            return $ret;
                        }
    ...
    public function restore_crypt_db($file,$local_path,$sub_task)
    {
        $restore_task=get_option('wpvivid_restore_task',array());
        ...
        if($ret['result']=='success')
        {
            ...
            if(isset($sub_task['options']['is_crypt_ex']) && $sub_task['options']['is_crypt_ex'] == '1')
            {
                $root_path = $this->transfer_path(WPvivid_Custom_Interface_addon::wpvivid_get_local_backup_abspath());
                $root_path = rtrim($root_path, '/');
                $root_path = rtrim($root_path, DIRECTORY_SEPARATOR);
                $ret=$this->extract($file_path,untrailingslashit($root_path));
                if($ret['result']!=='success')
                {
            ...
            else
            {
                ...
                {
                    unset($archive);
                    if(!isset($sub_task['is_check_crypt'])|| !$sub_task['is_check_crypt'])
                    {
    ...
    public function exec_sql($sql_file_name,$local_path,$sub_task)
    {
        ...
        global $wpdb;
        $this->db_method->set_foreign_key_check();
                ...
                else if(preg_match('#^\\s*DROP TABLE #', $query))
                {
                    ...
                    else
                    {
                        if($this->is_migrate&&!empty($sub_task['exec_sql']['current_table']))
                        {
                            ...
                            {
                                if($this->is_mu_single_og_table($sub_task['exec_sql']['current_old_table']))
                                {
                            ...
                            else if($this->is_og_table($sub_task['exec_sql']['current_old_table']))
                            {
                                $sub_task['exec_sql']['current_need_replace_table']=true;
    ...
    public function finish_restore_db($sql_files,$local_path,$sub_task)
    {
        ...
        foreach ($sql_files as $sql_file_name=>$sql_file)
        {
            ...
            $tmp_sql_zip_file=$local_path.$sql_file_name.'.zip.decrypted.zip';
            if(file_exists($tmp_sql_zip_file))
            {
                @unlink($tmp_sql_zip_file);
            }
        }
    ...
    public function rename_db($sub_task)
    {
        ...
        if($restore_db_reset)
        {
            foreach ($tables as $table)
            {
                ...
                if (substr($table->Name, strlen($wpdb->prefix)) === 'wpvivid_options')
                {
                    continue;
                }
