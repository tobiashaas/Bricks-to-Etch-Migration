...
class WPvivid_Mysqldump_Addon
{
    ...
    public function __construct($task,$dump_setting)
    {
        ...
        $this->dumpSettings['init_commands'][] = "SET NAMES " . WPvivid_Mysqldump_Addon::UTF8MB4;
    ...
    private function listValues($tableName,$index)
    {
        ...
        $primary_key = $this->find_primary_key($tableName);
        if(isset($this->dumpSettings['backup_database_use_primary_key']) && $this->dumpSettings['backup_database_use_primary_key'])
        {
        ...
        if($dbType=='wpdb')
        {
            ...
            {
                if ($use_offset) {
                    $query = $stmt . " LIMIT $limit_count OFFSET $start";
                } else {
                    $where = strpos($stmt, 'WHERE') !== false ? ' AND' : ' WHERE';
                    $query = $stmt . $where . " `$primary_key` > $last_id ORDER BY `$primary_key` ASC LIMIT $limit_count";
                }
                ...
                foreach ($resultSet as $row)
                {
                    ...
                    if (!$use_offset) {
                        $last_id = max($last_id, $row[$primary_key]);
                    }
    ...
    private function find_primary_key($tableName)
    {
        global $wpdb;
        ...
        $prepared_query = $wpdb->prepare($query, $tableName, $wpdb->dbname);
