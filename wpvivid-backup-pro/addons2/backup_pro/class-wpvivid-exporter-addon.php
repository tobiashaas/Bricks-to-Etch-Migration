...
class WPvivid_Exporter_task_Ex
{
    ...
    public function new_backup_task($options)
    {
        ...
        $this->task['options']['backup_options']['dir']=WPvivid_Custom_Interface_addon::wpvivid_get_content_backupdir();
                ...
                if($bfound)
                {
                    ...
                    if($attach_meta!=false)
                    {
                        ...
                        else
                        {
                            ...
                            $wpvivid_backup_pro->wpvivid_pro_log->WriteLog('attach_meta size not found id:'.$attachment_id,'notice');
                        }
                    }
                }
            }
...
class WPvivid_Exporter_Ex
{
    ...
    public function export($task_id)
    {
        ...
        {
            @set_time_limit(900);
            $wpvivid_backup_pro->wpvivid_pro_log->WriteLog('Prepare to export post '.$next['file_name'],'notice');
            $wpvivid_backup_pro->wpvivid_pro_log->WriteLog('Size:'.sizeof($next['next_post_ids']),'notice');
            ...
            $wpvivid_backup_pro->wpvivid_pro_log->WriteLog('Finished to export post '.$next['file_name'],'notice');
            if($ret['result']=='success')
            {
                $wpvivid_backup_pro->wpvivid_pro_log->WriteLog('Prepare to zip file '.$next['file_name'],'notice');
                ...
                $wpvivid_backup_pro->wpvivid_pro_log->WriteLog('Finished to zip file '.$next['file_name'],'notice');
                if($ret['result']!='success')
                {
                    $wpvivid_backup_pro->wpvivid_pro_log->WriteLog('Failed to zip post '.$next['file_name'].' '.json_encode($ret),'notice');
                    return $ret;
                }
            ...
            {
                $wpvivid_backup_pro->wpvivid_pro_log->WriteLog('Failed to export post '.$next['file_name'].' '.json_encode($ret),'notice');
                return $ret;
            }
            $next=$this->task->get_next_posts();
        }
    ...
    public function export_post_to_xml($posts_ids,$attachment_ids,$file_name,$export_type)
    {
        ...
        //$files=array();
        $export_folder = WPvivid_Custom_Interface_addon::wpvivid_get_local_backup_abspath().DIRECTORY_SEPARATOR.WPVIVID_IMPORT_EXPORT_DIR_ADDON;
    ...
    private function zip_media_files($xml_file,$files,$file_name,$export_type,$json_info=false)
    {
        ...
        $path=WPvivid_Custom_Interface_addon::wpvivid_get_local_backup_abspath().DIRECTORY_SEPARATOR.WPVIVID_IMPORT_EXPORT_DIR_ADDON.DIRECTORY_SEPARATOR.$file_name.'_export_'.$export_type.'.zip';
